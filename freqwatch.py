#DetailTech fork of freqwatch, updating to python3
...
class Scanner:
    def __init__(self, devid, freqs, squelch, ppm, params):
        ...
        try:
            self.db = pymysql.connect(host=params.getparam('db_ip'),
                                      port=int(params.getparam('db_port')),
                                      user=params.getparam('db_user'),
                                      password=params.getparam('db_pass'),
    ...
    def worker(self):
        ...
        while not stop.isSet():
            p = Popen([self.cmd, "-d {}".format(self.devid), "-f {}".format(self.freqs),
                       "-1", "-p {}".format(self.ppm)], stdout=PIPE, stderr=self.devnull, universal_newlines=True)

            data = p.communicate()[0].strip()
            rc = p.returncode

            if rc != 0:
                print("rtl-power exited with error code that was not 0({}); thread terminating".format(rc))

            for tmp in data.split('\n'):
                if stop.isSet():
                    return
                try:
                    d, t, freq_low, freq_high, freq_step, samples, raw_readings = tmp.split(', ', 6)
...
class Collector:
    def __init__(self, params):
        ...
        for s in scanners.split(','):
            ...
            if freqs is None:
                print("Is there a frequency entry for each device you've specified in 'scanners' in the configuration?")
                sys.exit(ERR)
